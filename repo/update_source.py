...
DEFAULT_SIDELOAD_KEYWORD = "sideload"
...
def load_config(config_path: str) -> AppConfig:
    ...
    try:
        ...
        return {
            ...
            "sideload_asset_keyword": config_data.get(
                "sideload_asset_keyword", DEFAULT_SIDELOAD_KEYWORD
            ),
        }
...
def find_latest_installable_release(
    releases: List[GitHubRelease], keyword: str = DEFAULT_SIDELOAD_KEYWORD
) -> GitHubRelease:
    """
    Find the newest release with a sideload IPA asset, falling back to any IPA
    asset if no release has one matching the keyword.

    Args:
        releases: List of GitHub releases
        keyword: Substring (case-insensitive) that identifies the sideload IPA
            asset, used to prefer it over other .ipa assets (e.g. a separate
            App Store/TestFlight build) in the same release
    ...
    for release in sorted_releases:
        if any(
            keyword.lower() in asset["name"].lower() and asset["name"].endswith(".ipa")
    ...
    for release in sorted_releases:
        if any(asset["name"].endswith(".ipa") for asset in release["assets"]):
            logging.info(
                f"Latest installable release (no '{keyword}' match, using fallback .ipa): {release['tag_name']}"
            )
            return release
...
def find_download_url_and_size(
    release: GitHubRelease, keyword: str = DEFAULT_SIDELOAD_KEYWORD
) -> Tuple[Optional[str], Optional[int]]:
    """
    Find the download URL and size for a release's sideload IPA file, preferring
    an asset matching the keyword over any other .ipa asset in the same release.

    Args:
        release: The GitHub release
        keyword: Substring (case-insensitive) that identifies the sideload IPA asset

    Returns:
        tuple: (download_url, size) or (None, None) if not found
    """
    ipa_assets = [asset for asset in release["assets"] if asset["name"].endswith(".ipa")]

    keyword_matches = [asset for asset in ipa_assets if keyword.lower() in asset["name"].lower()]
    if keyword_matches:
        asset = keyword_matches[0]
...
def update_json_file(
    ...
    app = data["apps"][0]
    keyword = config["sideload_asset_keyword"]
    ...
    for release in fetched_data_all:
        ...
        download_url, size = find_download_url_and_size(release, keyword)
    ...
    download_url, size = find_download_url_and_size(fetched_data_latest_installable, keyword)
...
def main() -> None:
    ...
    try:
        ...
        fetched_data_latest_installable = find_latest_installable_release(
            fetched_data_all, config["sideload_asset_keyword"]
        )
        update_json_file(
            config,
            config["json_file"],
